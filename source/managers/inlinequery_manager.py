...
class InlineQueryManager:
    ...
    def manage(self):
        ...
        for song in result_song_list:
            result_display_list.append(InlineQueryResultArticle(id=song.url, title=song.title, input_message_content=InputTextMessageContent(song.lyrics), url=song.url, hide_url=True, description=song.artist, thumb_url=song.thumbnail_url, thumb_height=640, thumb_width=640))
            i += 1
